...
| 5b. Prompt footer parity                   | in progress | The prompt footer is now rendered as a distinct layer beneath the input box, with mode/activity state and shortcut hints split out of the prompt body. Token-warning and cost-threshold behavior are still pending.                                                                                                                                          |
...
- Remaining in Phase 5b: add token-warning and cost-threshold style footer notices when the relevant state exists locally.
- Next: continue Phase 5b by deciding whether those footer warnings can be derived entirely in the TUI or need additional engine/state plumbing.
