...
| 4. Transcript/message-row parity           | in progress | The transcript now uses a shared row wrapper, distinct row renderers, continuation-aware message labels, and assistant row metadata stamped from reducer state. Virtualized rendering and fuller upstream row parity are still pending.       |
...
- Remaining in Phase 4: bring the transcript closer to the upstream row system by refining spacing/metadata behavior and adding virtualized rendering for long sessions.
- Next: continue Phase 4 by evaluating a long-session virtualization approach now that row headers and metadata are handled closer to upstream.
