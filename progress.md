...
| 5b. Prompt footer parity                   | in progress | The prompt footer is now rendered as a distinct layer beneath the input box, with mode/activity state and shortcut hints split out of the prompt body. A best-effort context warning is now shown from local model/token state, but cost-threshold behavior still lacks a real local source of truth.                                                        |
...
- Completed: added a best-effort context warning row to the prompt footer using the local model preset and accumulated token counts, matching the upstream footer's focus on context pressure rather than generic prompt hints alone.
- Remaining in Phase 5b: cost-threshold style footer notices still need either an actual local threshold setting or explicit engine/UI state, since there is no trustworthy source for them in the current TUI.
- Next: decide whether to finish Phase 5b with the current footer behavior and carry cost-threshold support into Phase 6 follow-up, or add new threshold state first.
