...
| 4. Transcript/message-row parity           | in progress | The transcript now uses a shared row wrapper and distinct renderers for user text, assistant text, live thinking, live assistant output, tool rows, and grouped read/search rows. Virtualized rendering and fuller upstream row parity are still pending. |
...
- Next: continue Phase 4 by pushing more transcript behavior into row-level renderers and evaluating the virtualization step.
