...
| 4. Transcript/message-row parity           | in progress | The transcript now uses a shared row wrapper, distinct row renderers, continuation-aware message labels, assistant row metadata stamped from reducer state, and an anchored long-session render cap modeled on the upstream non-virtual fallback path. Full virtualization and remaining parity polish are still pending. |
...
- Remaining in Phase 4: add true virtualized transcript rendering if the TUI gains the scroll/fullscreen primitives needed for a real `VirtualMessageList` equivalent, then finish any remaining spacing polish.
- Next: decide whether to build transcript scrolling infrastructure in the local TUI or treat the anchored render cap as the Phase 4 endpoint for the current architecture.
